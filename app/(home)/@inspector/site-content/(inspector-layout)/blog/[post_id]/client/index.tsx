...
export default function BlogInspectorClient({ postFromServer, tagsFromServer, addTagAndBindToPost, removeTagFromPost }: { postFromServer: any, tagsFromServer: any, addTagAndBindToPost: any, removeTagFromPost: any }) {
  ...
  const [tags, setTags] = useState<TagToken[]>(tagsFromServer || []);
  const [status, setStatus] = useState<"draft" | "published">(postFromServer.status || "draft");
  const [title, setTitle] = useState<string>(postFromServer.title || "");
  const [excerpt, setExcerpt] = useState<string>(postFromServer.excerpt || "");
  ...
  }, [postFromServer, title, excerpt, status, tags]);


  async function handleAddTag(tag: TagToken) {
    // console.log(tag)
    await addTagAndBindToPost({ title: tag.title, slug: tag.slug, postId: postFromServer.id })
    setTags([...tags, tag])
    // console.log(tags)
  }

  async function handleRemoveTag(tag: TagToken) {
    // console.log(tag, postFromServer.id)
    setTags(tags.filter(t => t.id !== tag.id))
    await removeTagFromPost({ tagId: tag.id, postId: postFromServer.id })
          ...
          >
            ...
            <div className="flex flex-row items-center gap-1">
              <span className="cursor-pointer font-medium items-center p-1 px-0 text-blue-500 text-xs   underline underline-offset-2 inline-block w-full">
                {"/" + slugify(postFromServer.title) + "-" + postFromServer.slug_uid}
              </span>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-[1fr_auto] items-center gap-2">
              ...
              <input
                ...
                placeholder="Blog yazısı başlığı..."
                  className=" border rounded-[0.15rem] p-1  w-full text-xs bg-background dark:bg-input/20"
              />
            ...
            <div className="grid grid-cols-[1fr_auto] items-baseline gap-2">
              ...
              <textarea
                ...
                rows={5}
                  className=" border rounded-[0.15rem] p-1  w-full text-xs bg-background dark:bg-input/20 resize-none"
              />
            ...
            />
            <Inspector.Comment>Etiketleri virgülle ayırın veya enter tuşuna basın</Inspector.Comment>
          </div>
        </div>
      ) : (
