...
function IsolatedInput({ className, containerClassName, errorPopup,performSaveRequest,saveRequestLoading, type, ...props }: ComponentProps<"input"> & { containerClassName?: string, errorPopup?: ReactNode, performSaveRequest?: (value: string) => void, saveRequestLoading?: boolean }) {
  const [value, setValue] = useState(props.value || '')
  React.useEffect(() => {
    handleSaveRequest()
  }, [saveRequestLoading])
  const handleSaveRequest = () => {
    performSaveRequest(value)
  }
  return (
    <div className={cn(
      "relative px-3 py-2 rounded-md border dark:hover:bg-input/30 cursor-pointer has-focus:cursor-text has-focus:ring-1 has-focus:ring-primary/80 border-input has-focus:border-primary/80 duration-100 shadow-xs dark:bg-input/20 dark:!border-0", containerClassName)}>
...
function MailInput({ className, containerClassName,innerItem, type, ...props }: ComponentProps<"input"> & { containerClassName?: string, errorPopup?: ReactNode, innerItem?: ReactNode }) {
  return (
    <div className={cn(
      "relative px-3 py-1 dark:bg-input/20 rounded-md border border-input has-focus:!border-primary/80 duration-100 dark:!border-0 dark:has-focus:!border-0  transition-[color,box-shadow]", containerClassName, innerItem && "flex flex-row  items-center gap-2 "
    )}>
      <input
        type={type}
        data-slot="input"
